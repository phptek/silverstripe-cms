...
class CMSSiteTreeFilter_StatusRemovedFromDraftPages extends CMSSiteTreeFilter {
	...
	public function pagesIncluded() {
		...
		foreach($pages as $page) {
			$statusFlags = $page->getStatusFlags();
			if(isset($statusFlags['removedfromdraft'])) {
				$ids[] = array('ID' => $page->ID, 'ParentID' => $page->ParentID);
			}
		}
		return $ids;
...
class CMSSiteTreeFilter_StatusDraftPages extends CMSSiteTreeFilter {
	...
	public function pagesIncluded() {
		...
		foreach($pages as $page) {
			$statusFlags = $page->getStatusFlags();
			if(isset($statusFlags['addedtodraft'])) {
				$ids[] = array('ID' => $page->ID, 'ParentID' => $page->ParentID);
			}			
		}
...
/**
 * Filters pages which have a status "Modified".
 * 
 * @package cms
 * @subpackage content
 */
class CMSSiteTreeFilter_StatusModifiedPages extends CMSSiteTreeFilter {
	
	static public function title() {
		return _t('CMSSiteTreeFilter_StatusModifiedPages.Title', 'Status: Modified');
	}
	
	/**
	 * Filters out all pages who's status is set to "Modified".
	 * 
	 * @see {@link SiteTree::getStatusFlags()}
	 * @return array
	 */
...
class CMSSiteTreeFilter_StatusDeletedPages extends CMSSiteTreeFilter {
	...
	public function pagesIncluded() {
		...
		foreach($pages as $page) {
			$statusFlags = $page->getStatusFlags();
			if(isset($statusFlags['deletedonlive'])) {
				$ids[] = array('ID' => $page->ID, 'ParentID' => $page->ParentID);
			}			
		}
