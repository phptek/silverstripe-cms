...
class CMSSiteTreeFilterTest extends SapphireTest {
	...
	public function testStatusModifiedPagesFilter() {
		$modifiedPage = $this->objFromFixture('Page', 'page5');
		$modifiedPage->doPublish();
		$modifiedPage->Content = "Changed something.";
		$modifiedPage->write();
		$modifiedPage = Versioned::get_one_by_stage(
			'SiteTree', 
			'Stage', 
			sprintf('"SiteTree"."ID" = %d', $modifiedPage->ID)
		);

		$f = new CMSSiteTreeFilter_StatusModifiedPages();
		$f->pagesIncluded();

		$this->assertTrue($f->isPageIncluded($modifiedPage));
		
		// Ensures empty array returned if no data to show
		$modifiedPage->delete();
		$this->assertEmpty($f->isPageIncluded($modifiedPage));		
	}			
